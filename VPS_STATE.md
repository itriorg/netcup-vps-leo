...
Default routed: deny
Allowed inbound: TCP 22, TCP 80, TCP 443, UDP 51820
```

However, a 2026-09-17 observation reported host listeners for TCP 3389, TCP 8443, and telemetry ports 4317/8125 in addition to 22/80/443. This is a security-relevant `CONFLICT`, not a harmless detail.
...
Do not close or expose these ports until each listener is mapped to an intended service and a recovery path is confirmed. RDP 3389 is especially high priority if not explicitly required.
...
- Do not place blanket Cloudflare Access in front of n8n before webhook design/testing.
- Do not place browser-only Access in front of Nextcloud before desktop/mobile/WebDAV/CalDAV/CardDAV testing.
...
- Test webhook behavior before any Cloudflare Access policy.
...
Status: `PENDING` application authentication and access-boundary review. Do not expose it publicly or assume container/network privacy replaces application authentication.
...
Older profiles used `.2` through `.6`. Migration was staged, but completion and old-peer retirement remain `PENDING`.
...
sudo docker compose --project-directory /srv/stack/n8n \
  -f /srv/stack/n8n/compose.yml exec -T postgres \
  pg_restore --list < "$latest_dump" | head -20
```

Do not schedule recurring dumps until the manual dump and validation succeed.
...
9. Consider Cloudflare Access only after service-specific client/webhook testing.
...
- [ ] Cloudflare Zero Trust Access for n8n and Nextcloud.
